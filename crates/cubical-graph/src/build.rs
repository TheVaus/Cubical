...
impl Builder {
    ...
    fn file_node(&self, path: &str) -> Option<NodeId> {
        self.get(NodeKind::Note, path)
            .or_else(|| self.get(NodeKind::Attachment, path))
    }
}
...
pub async fn build_model(conn: &IndexConn) -> Result<GraphModel, GraphError> {
    ...
    while let Some(r) = rows.next().await? {
        ...
        let Some(source) = b.file_node(&source_path) else {
            continue;
        };
        match target_path {
            Some(t) => {
                let Some(target) = b.file_node(&t) else {
                    continue;
                };
