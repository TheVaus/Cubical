...
pub async fn list_dangling_links(
    ...
    let files = cubical_index::all_file_paths(vault.index()).await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn a_differently_cased_title_key_is_not_a_title() {
        let (dir, vault, state) = vault_with(&[
            ("src.md", "see [[plan]]\n"),
            ("notes/plan.md", "one\n"),
            ("archive/roadmap.md", "---\nTitle: plan\n---\ntwo\n"),
        ...
        let resp = list(&state).await;
        assert_eq!(resp.groups.len(), 1);
        assert_eq!(shape(&resp.groups[0]), Vec::<(&str, &str)>::new());
    }
