...
**Current branch `feat/terminal`: the embedded terminal is BUILT (B1–B4), gate green, not merged.** Real PTY + xterm.js, non-singleton `{kind:"terminal"}` tabs, `cubical` on the child `PATH` so engine-routed ops are available but not mandatory, all behind `plugins.terminal_enabled` (**default off**). **Interception is impossible and not attempted** — see [`foundation.md`](docs/architecture/foundation.md) §2.1 (native-capability rule for gateway features: opt-in/default-off, cannot compromise integrity when abused, auditable) and §2.2 (convergence over interception). Owned by [spec](docs/superpowers/specs/2026-07-30-terminal-design.md) + [plan](docs/superpowers/plans/2026-07-30-terminal.md); the plan's STATUS block carries what landed and the two integration defects fixed. **Open before merge:** no terminal has been opened in a real GUI — this is the one feature where the Tauri smoke actually matters, since process leaks are the stated risk. **Removing the console is still a separate later commit**, not done.
