...
**Next session's task:** The file watcher per `docs/layer-0-spec.md` §6 — `notify` 6.x with `RecommendedWatcher`, debounced 100ms, routed to a stub handler that updates `files.last_seen` / `mtime_unix` / `content_hash` and emits `vault:file-changed` (created / modified / removed / renamed). Wire watcher lifecycle into `OpenVault` so `close_vault` stops it cleanly. *Before* writing watcher code: complete the L0 §12 DoD #4 smoke test that this session deferred — `cargo tauri dev`, open a real 10-file folder, verify the five points listed above. The watcher work depends on a confirmed-working scan path.
