...
**Current branch — `feat/design-system-migration` (UNMERGED; `scripts/check.sh` green modulo the documented watcher flake):** making `design-system/` the app's single source of truth for **tokens AND components** (`@ds` alias; `ui/` borrows every component). **Phase B is complete and all three Phase-D spot-check deltas are now settled** (live under `cargo tauri dev`): PendingRewrites statusbar fit and ChipList reflow PASS, and the **BooleanCell whole-row/label click regression is FIXED** (2026-07-17) via an additive Toggle `showLabel` slot in `design-system/` — clicking the `true/false` text now flips the value both ways and writes to disk (live-verified). No open UI items from that pass. **Phase C: slices C1 (context menu → DS `Menu`) and C2 (delete-confirm → DS `Modal`) are DONE & live-verified** (2026-07-17; added `MenuItem.danger`, DS Modal ARIA + `size`/`placement`, killed bespoke `contextMenuItemStyle`). A **Phase-C fit audit** established the remaining overlays are deliberately bespoke (Settings modal two-pane, OmniBar ranked palette, popovers — no DS target fits), so the DS-backed overlay migrations are exhausted. Remaining: **rest of C** (inline-primitive sweep: ~31 bespoke `<button>`/`<input>` in the still-bespoke wrappers) and **D** (gut the 679-line `layout.css`, re-verify live). ~62% done by effort. **Read [`docs/superpowers/2026-07-17-ds-migration-progress.md`](docs/superpowers/2026-07-17-ds-migration-progress.md) before touching this branch** — it owns the DS self-containment rule, the Properties draft-guard invariant, the deliberately-bespoke list, and the settled delta results.
