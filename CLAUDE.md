...
**Current branch — `feat/design-system-migration` (UNMERGED; `scripts/check.sh` green modulo the documented watcher flake):** making `design-system/` the app's single source of truth for **tokens AND components** (`@ds` alias; `ui/` borrows every component). **Phase B is complete and all three Phase-D spot-check deltas are now settled** (live under `cargo tauri dev`): PendingRewrites statusbar fit and ChipList reflow PASS, and the **BooleanCell whole-row/label click regression is FIXED** (2026-07-17) via an additive Toggle `showLabel` slot in `design-system/` — clicking the `true/false` text now flips the value both ways and writes to disk (live-verified). No open UI items from that pass. **Phase C: slices C1 (context menu → DS `Menu`), C2 (delete-confirm → DS `Modal`), C3 (tree-header ＋/🗀 → DS `IconButton`) are DONE & live-verified** (2026-07-17; added `MenuItem.danger`, DS Modal ARIA + `size`/`placement`; killed bespoke `contextMenuItemStyle` + dead `tree-header__action`). A **Phase-C fit audit** established the remaining overlays are deliberately bespoke (Settings modal two-pane, OmniBar ranked palette, popovers — no DS target fits); the DS-backed overlay migrations are exhausted and the inline-primitive sweep's migratable remainder is a diffuse tail (external-edit banner buttons, OmniBar input — see the C2-plan inventory). **Phase D (`layout.css` gut) is DONE (2026-07-18): the "gut" was near-empty by design** — B/C's collision strategy already deletes each migrated element's dead CSS as it goes, so a defined-vs-applied class audit found exactly ONE orphan (`.statusbar__spacer`, removed → 676 lines); the rest is live layout scaffolding + the deliberately-kept Settings `.modal*`/`.set-info-*`. Do NOT delete more of `layout.css` to shrink it. Live-verified under `cargo tauri dev` (statusbar three-region layout intact; vault byte-for-byte). **All structural campaign work is now complete**; only the optional deferred inline tail (external-edit banner buttons, OmniBar input) remains. ~66% done by effort. **Read [`docs/superpowers/2026-07-17-ds-migration-progress.md`](docs/superpowers/2026-07-17-ds-migration-progress.md) before touching this branch** — it owns the DS self-containment rule, the Properties draft-guard invariant, the deliberately-bespoke list, the Phase-D finding, and the settled delta results.

**On `main` (16 commits, unpushed):** all Layer-4 feature work merged — recent-vaults store (the first global, non-vault state; app-shell-owned, pattern for future machine-local state), small-wins UI batch, configurable shortcuts, folder rename, raw-source coloring, minimap, create files/folders, property-ref interpolation, configurable status bar, rename durability journal, Core Plugins + portable `.cubical/config.toml`. Each is owned by its `docs/superpowers/specs|plans/*` pair. Typed properties (inline `# type:`) merged but **defaulted off** — slated for a vault-level type registry (`docs/architecture/planned.md` §14; issue #19). The **`l4` close-tag is the only structural open gate** — operator smoke, recipe `layer-4-spec.md` §9.5–9.6. Nothing user-requested is open; deferred-complex (untouched): dataview f(x) graphs, calendar, terminal, tabs (#20), other-file-type viewers. **Open bug:** intermittent `[[wikilink]]` non-render in Live Preview (raw mode fine, no pattern yet). **2 Dependabot alerts remain, blocked upstream:** `lru` #3 (low, tantivy 0.22), `glib` #1 (medium, Linux-GTK-only).
