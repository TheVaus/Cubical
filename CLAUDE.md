...
Current layer: 3 — Knowledge Graph (Sessions A + B done, Sessions C–K pending). Session B installed the Lezer inline rule for `[[…]]` / `![[…]]` (runs `before: "Link"`; emits a single `WikiLink` node), extended `decorations.ts` with off-cursor hide-brackets/anchor/display + visible-text-as-accent-link styling (cursor-line reveals raw, embeds carry a `⎘` indicator widget, unresolved targets render dashed-underline + `--c-warning`), and wired a per-vault `WikiLinkResolver` cache over the Session A `resolve_link` IPC (invalidated on `vault:file-changed`, surfaced to extensions via a CodeMirror `Facet`). Click routing: resolved links open the target via the existing file-open flow and scroll to heading anchors via `EditorApi.scrollToHeading`; unresolved links raise a centered modal offering to create the note at the resolved-by-convention `target.md` path. The L2 Session E raw-source toggle's compartment swap still suppresses every wiki-link decoration in lockstep.
Tests: 170 Rust + 161 vitest. L0 closed 2026-05-13 (`l0`); L1 closed 2026-05-09 (`l1`); L2 closed 2026-05-22 (`l2`).
Next: L3 Session C — Backlinks panel + right-sidebar shell (build-order §3, layer spec §2.3 + §8 Session C). The Session A `links` table + Session B resolver subscriptions are the substrate.
