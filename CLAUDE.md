...
**Current branch — `feat/design-system-migration` (23 commits, UNMERGED, HEAD `d48b9e0` docs-only; `scripts/check.sh` green at the last code commit):** making `design-system/` the app's single source of truth for **tokens AND components** (`@ds` alias; `ui/` borrows every component). **Phase B is complete and the Phase-D delta spot-check is done** (live under `cargo tauri dev`, 2026-07-17): PendingRewrites statusbar fit and ChipList reflow both PASS; the **BooleanCell whole-row/label click regression is confirmed still open** (needs an additive Toggle visible-label slot) — the one open UI item from that pass. Remaining: **C** (behavioral wrappers: editor / omnibar / file tree / statusbar, dialog shells → Modal, context menu → Menu) and **D** (gut the 679-line `layout.css`, re-verify live). ~50–55% done by effort. **Read [`docs/superpowers/2026-07-17-ds-migration-progress.md`](docs/superpowers/2026-07-17-ds-migration-progress.md) before touching this branch** — it owns the DS self-containment rule, the Properties draft-guard invariant, the deliberately-bespoke list, and the now-verified delta results.
