...
**Current branch `chore/dependabot-security-fixes`:** **Dependabot security sweep** — 19 of 21 alerts fixed, `npm audit` clean, all gates green; not yet merged to `main`/pushed (pending go-ahead). Key move: libsql switched to `default-features=false, features=["core"]`, dropping the entire remote/replication/tls stack (hyper/tonic/rustls-webpki/libsql-sqlite3-parser) that a no-cloud vault never uses — killed 5 Rust alerts incl. the one with no upstream patch. Plus tauri→2.11.1, and frontend vite 5→7 / vitest 2→3 / undici (all 13 npm alerts, dev-tooling only). Deliberately stayed on vite 7 not 8 (vite 8's rolldown bundler breaks vite-plugin-solid build). **2 alerts remain, blocked upstream** (not worth disproportionate major bumps): `lru` #3 (low, pinned by tantivy 0.22) and `glib` #1 (medium, Linux-GTK-only, pinned by tauri's gtk 0.18). `main` itself is **13 commits ahead of `origin/main` (unpushed)** once this branch merges.

Prior `main` head: **configurable shortcuts** (rebind the 3 keymaps from Settings → Shortcuts, per-vault `shortcuts.overrides` diff, live-applied; spec+plan `docs/superpowers/specs|plans/2026-07-05-configurable-shortcuts*`), plus two trivial fixes (empty-properties box hidden; false "changed outside Cubical" banner on create suppressed), plus folder rename (concurrent session).

**On `main`:** all recent feature work merged — **raw-source coloring** (`editor.colorize_raw_source`, default off), configurable shortcuts, folder rename, minimap (`editor.minimap_enabled`, default off), create files + folders, property-ref interpolation (default-on), configurable status bar, Live Preview touch-reveal, rename durability journal + deleted-file pruning + rename-coalescing, Core Plugins + portable `.cubical/config.toml` settings, all Layer 4. Typed properties (inline `# type:`) merged but **defaulted off** — slated for a vault-level type registry (`docs/architecture/planned.md` §14; issue #19). The **`l4` close-tag is the only structural open gate** — operator smoke, recipe `layer-4-spec.md` §9.5–9.6. **User-requested backlog** (not started; one feature/session): in-app vault-switcher popup, editor back-button + shortcut, more configurable shortcuts (wikilink-at-cursor etc.), muted file-type labels in sidebar, clearer settings/instructions; deferred-complex: dataview f(x) graphs, calendar, terminal, tabs (#20), other-file-type viewers (png/svg/pdf/txt). **Open bug:** intermittent `[[wikilink]]` non-render in Live Preview (raw mode fine, no pattern yet). Deferred (GitHub Issues): per-occurrence search (#21).
...
**Tests:** 709 vitest + 555 Rust. Gates: `scripts/check.sh` (fmt/clippy/test, tsc, vitest, build, docs) — all green. Layer status/tags/dates: `docs/build-order.md`.
