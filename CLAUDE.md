...
**Next session's task:** Vault open + non-blocking scan in `cubical-core` + `cubical-app` per `docs/layer-0-spec.md` §3 and §8. `Vault::open(path)` validates the directory, ensures `.cubical/`, and calls `cubical_index::open_index`. A separate `scan(vault, cancel, progress)` Tokio task walks the tree (skipping `.cubical/`, `.git/`, `node_modules/`, `.`-prefixed dirs), dispatches files through the registry, hashes via the handler, and upserts into `files`. Tauri shims for `open_vault`, `cancel_vault_scan`, `get_vault_info`, `list_files`, `close_vault` forward to pure handlers in `commands/vault.rs`; `vault:scan-progress` and `vault:scan-complete` events stream from `events.rs`. Frontend wires `ui/src/api/ipc.ts` typed wrappers and an "Open Vault" flow in `App.tsx` using `@tauri-apps/plugin-dialog`. The file watcher (`notify` 6.x) is the session after.
