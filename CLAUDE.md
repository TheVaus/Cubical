...
**Current branch — `feat/design-system-migration` (21 commits, UNMERGED, `scripts/check.sh` green at HEAD):** making `design-system/` the app's single source of truth for **tokens AND components** (`@ds` alias; `ui/` borrows every component). **Phase B is complete**; C (behavioral wrappers: editor / omnibar / file tree / statusbar, dialog shells → Modal, context menu → Menu) and D (gut the 679-line `layout.css`, live `cargo tauri dev` verify) remain. **Read [`docs/superpowers/2026-07-17-ds-migration-progress.md`](docs/superpowers/2026-07-17-ds-migration-progress.md) before touching this branch** — it owns the DS self-containment rule, the Properties draft-guard invariant, the deliberately-bespoke list, and the deltas Phase D must settle live.
