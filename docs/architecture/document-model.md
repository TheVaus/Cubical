...
**Grammar, one owner:** an ID is ASCII — `[A-Za-z_][A-Za-z0-9_-]*` — and the owner is `cubical_core::is_valid_block_id`, with `block_id_at_line_end` owning what counts as a *trailing* ID (caret at line start or after whitespace, trailing whitespace tolerated). Minting, renaming, indexing, decoration, anchor scroll and autocomplete are all consumers: they import the owner or, across the language boundary, agree with it through `crates/cubical-core/tests/fixtures/block-ids.json`, which the Rust `block_id_grammar` integration test and the TS `blockId.test.ts` both read. ASCII is deliberate and narrower than the Unicode grammar this section claimed until #305. An ID is not prose: it is minted by the app rather than typed (`unique_id` hashes path and position), and no build has ever indexed or decorated a Unicode one. It is also the one token in a note that another machine must reproduce byte-for-byte to resolve a reference, so it is the worst place to inherit the normalisation hazard a portable vault still has open — `^café` written NFC and NFD are different bytes that render identically, and nothing normalises them (#123). Tag and note names, which the user *does* choose, keep their Unicode grammar and fold through `fold_name`.
...
**Casing** is case-insensitive for matching, case-preserving for display. The canonical display form is whichever case was typed first in the vault; a tag-edit UI lets the user override. The fold has one owner, `cubical_index::fold_name` — the same fold vault names use, for the reason [`engine-ipc.md`](../implementation/engine-ipc.md) gives — applied in Rust on the way *into* the `tag_fold` column. Tag pages, wikilink-free tag autocomplete and the dataview `FROM #tag` source all compare that column and never `tag_path`, which exists for display; tag *rename* is the exception and matches `tag_path` literally (#310). Matching is by equality and range, not `LIKE`, so `idx_tags_fold` is readable: a BINARY index serves neither a `LIKE` pattern nor a function of the column. An empty needle matches nothing rather than everything — the empty string is the not-yet-backfilled marker, not a tag.
