...
2. **Performance is a feature, not a polish item.** Every architectural choice is measured against latency at the keystroke, scroll, and search. "Fast enough" is not the bar. "Imperceptible" is. This commitment is held to a measured number, not an adjective — the bar below is the owner; `CLAUDE.md` links to it.
   ...
   **The graph-layout bar.** A full force-directed layout of a 10,000-node graph (`cubical-graph`'s default 300 iterations, Barnes-Hut repulsion) must stay under **5.5 s**. Measured median on the same 10-core machine: **2.62 s** — the ceiling is set at roughly 2x observed, the same way the scan ceilings are. Reproduce with `cargo run --release -p cubical-graph --example layout_bench -- 10000`; the harness generates its own deterministic fixture and touches no vault. This is a one-time convergence cost paid when the graph opens, not a per-frame cost: pan and zoom rewrite a camera uniform and never re-run the simulation.
