...
2. **Performance is a feature, not a polish item.** Every architectural choice is measured against latency at the keystroke, scroll, and search. "Fast enough" is not the bar. "Imperceptible" is. This commitment is held to a measured number, not an adjective — the bar below is the owner; `CLAUDE.md` links to it.
   ...
   **The bar.** A cold scan-and-index (`Vault::open` + `SearchHandle::open` + full `scan`) of a synthetic fixture vault must stay under **13 s** at 10,000 notes and **1.5 s** at 1,000 notes. Measured medians on a 10-core M1 Pro at the time of writing: **6.6 s** and **0.69 s** — the ceilings are set at roughly 2x observed so they pass today and ratchet down, never up. Reproduce with `cargo run --release -p cubical-engine --example scan_bench -- <fixture-dir> <note-count>`; the harness generates its own deterministic fixture. It deletes `.cubical/` before every run, so it refuses any directory that is non-empty and lacks its own `.scan-bench-fixture` marker — pointed at a real vault it would destroy exactly the non-derivable state named in commitment 1. Tantivy is ~a third of the budget and scales with core count, so a CI ceiling must be measured on the runner rather than inheriting this number.

   **The graph-layout bar.** A full Fruchterman-Reingold layout of a 10,000-node graph must stay under **5.5 s**. Measured median on the same 10-core M1 Pro: **2.72 s** — ceiling set at roughly 2x observed, on the same ratchet-down-never-up rule as the scan bar. Reproduce with `cargo run --release -p cubical-graph --example layout_bench -- 10000`; the harness generates its own deterministic in-memory fixture and never touches a vault. This number is why the layout runs in Rust rather than as a GPU compute pass — see [#64](https://github.com/TheVaus/Cubical/issues/64).
...
- A feature toggle changes behaviour and derived state only — never the `.md` source of truth or the vault's portability. Switching a feature off leaves the vault byte-identical; that feature's derived state stays disposable (commitment 1), whether it is dropped or kept warm.

**On composability (commitment 4) — scope and honesty.** A direction realized incrementally, and *most* features, not all. The always-on substrate — vault, canonical AST (`document-model.md`), index, IPC, among others — is bedrock, not a toggle; blocks stack on top of it. That list is illustrative: the operative substrate set, backend and frontend, is the census in `scripts/domain-boundaries.json`, enforced by [`domain-scoped-dependencies`](../principles/domain-scoped-dependencies.md), the same way `scripts/dependency-boundary.json` is the operative form of [`crate-separation`](../principles/crate-separation.md). Blocks form a **dependency graph**, not free stacking: a block can't be active while a block it depends on is off (backlinks need the link index; embeds need link resolution). The seed mechanism already exists — the Core Plugins registry + `.cubical/config.toml` `plugins.*` toggles (e.g. `dataview_enabled`) — graduating into the full plugin ABI ([`planned.md`](planned.md) §8). The cost to respect: every toggle multiplies the interaction and test surface, so tested **default sets** and known-good combinations matter more than raw togglability.
...
**Editor surface:** CodeMirror 6 + Lezer. CodeMirror handles input, selection, IME, accessibility, and decoration; Lezer provides incremental markdown parsing for Live Preview. The Lezer markdown grammar is the editor's parser. Both ship as families of small packages rather than one library, so the manifest names the packages the editor actually imports: `@codemirror/state` and `@codemirror/view` (document model and rendering), `@codemirror/language` (syntax-tree access and highlight styles), `@codemirror/commands` (the default keymap and undo history), `@codemirror/lang-markdown` plus `@codemirror/autocomplete` (markdown mode and wikilink completion), and `@lezer/common`, `@lezer/markdown`, `@lezer/highlight` (tree cursors, the grammar extensions Cubical adds, and highlight tags). The `codemirror` meta-package is deliberately not among them: it is a curated re-export bundle that drags in `@codemirror/search`, which Cubical does not use, and an import that resolves only through a meta-package's hoist stops resolving the day that bundle's contents change.
...
**Child-process control (unix):** `nix`, with only the `signal` and `process` features. The embedded terminal reaps its children gracefully — SIGTERM, then SIGKILL after a grace period — which needs `kill(2)` and `waitpid(2)`; `std::process` offers only an immediate kill and no way to wait with a timeout. Unix-only, declared under `[target.'cfg(unix)'.dependencies]`; Windows uses the `portable-pty` handle instead.
