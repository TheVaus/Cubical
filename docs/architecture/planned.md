...
- ~~**Typed Properties (Obsidian-style).**~~ **Shipped but defaulted off, pending a storage rework.** A property-type layer over YAML frontmatter — Text / Number / Checkbox / Date / List — for type-aware editors, real-typed sort/query, and lossless round-trip. Frontmatter **stays YAML** (native scalar + date types, ecosystem-portable; format rationale in `docs/superpowers/archive/specs/2026-06-14-l4-d-dataview-design.md`). The shipped v1 stores a property's type as an inline `# type:` comment (spec/plan: `docs/superpowers/{specs,plans}/2026-06-17-typed-properties-inline-comments*`) — but that puts app metadata in the `.md` source of truth, against our non-negotiables, so the feature is **defaulted off** (`properties.typed_enabled` absent → false). The intended replacement is a **vault-level type registry** in `.cubical/` (keeps markdown pristine; types are derived state in the derived layer): design + migration path in [`docs/superpowers/specs/2026-06-20-typed-properties-vault-registry-design.md`](../superpowers/specs/2026-06-20-typed-properties-vault-registry-design.md). Build the registry before promoting typed properties default-on or letting sort/query/Dataview depend on type storage. Promote to its own architecture section (or `document-model.md`) when that lands.

- ~~**Property-reference interpolation.**~~ **Shipped (default-on core plugin `plugins.property_refs_enabled`).** Inline, read-only, display-time interpolation of a frontmatter scalar: `[[Gandalf.age]]` (cross-file) and `[[.age]]` (self). The dot is the property separator (split at the first dot, top-level key only), so dotted note names aren't `[[ ]]`-linkable — the UI blocks new ones and flags existing ones. v1 renders the **raw scalar** (no type formatting), so it takes **no** dependency on the typed-properties registry above and can evolve independently; type-aware formatting is a later additive layer that *would* lean on the registry. Self-refs resolve from the open document's frontmatter; cross-file refs resolve on-demand through the `get_property` IPC behind an embed-style caching resolver (never indexed). Design + plan: [`docs/superpowers/specs/2026-06-20-property-reference-interpolation-design.md`](../superpowers/specs/2026-06-20-property-reference-interpolation-design.md), [`docs/superpowers/plans/2026-06-20-property-reference-interpolation.md`](../superpowers/plans/2026-06-20-property-reference-interpolation.md).
