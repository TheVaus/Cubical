...
- ~~**Typed Properties (Obsidian-style).**~~ **Now live — in design/build on `feat/typed-properties`** (spec/plan: `docs/superpowers/{specs,plans}/2026-06-17-typed-properties-inline-comments*`). A property-type layer over YAML frontmatter — Text / Number / Checkbox / Date / List — for type-aware editors, real-typed sort/query, and lossless round-trip. Frontmatter **stays YAML** (native scalar + date types, ecosystem-portable; format rationale in `docs/superpowers/archive/specs/2026-06-14-l4-d-dataview-design.md`). Application-level type registry + UI, not a file-format change. Promote to its own architecture section (or `document-model.md`) when it merges.
