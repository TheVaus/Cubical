...
### 9.15 Session J.1 (partial) — Pending Rewrites infrastructure

**Status — partial. 2026-05-31.** The infrastructure half of J.1 landed (migration + index module + pure materializer + materialize-on-read wired into every effective-content reader + a per-target flush helper). The IPC half (rename / flush / count / undo handlers + flush triggers + watcher own-write hash gate + IPC registration + `ipc.ts` stubs) did NOT land — the subagent driving it hit a session limit mid-execution and produced a partially-applied state. Rolled back the partial chain to keep the tree green; what was already committed is the infrastructure surface only.

The brainstorm + design + split prompts are unchanged; the locked design at [`docs/superpowers/specs/2026-05-31-l3-session-j-pending-rewrites-design.md`](../superpowers/specs/2026-05-31-l3-session-j-pending-rewrites-design.md) still holds. The "Rename IPCs / Flush + helpers / Backend own-write hash gate / Flush triggers / IPC registration / J.1 frontend stub" sections of that spec are the unfinished work; the prompt at [`docs/superpowers/prompts/l3-session-j1-pending-rewrites-backend.md`](../superpowers/prompts/l3-session-j1-pending-rewrites-backend.md) can drive a fresh window through them.

**What landed.**

*Migration 006 + `cubical-index::pending` module.* Locked schema from `document-model.md` §5.7. `pending_rewrites(id, target_file, rewrite_kind, old_token, new_token, created_at, rename_op_id)` + two indexes (`idx_pending_target`, `idx_pending_op`); no FK on `target_file` (a row targeting a since-deleted file silently drops on flush). `HIGHEST_KNOWN_VERSION = 6`; the `v6_applies_on_top_of_existing_v5_database` test pins the upgrade path. Query module exposes the full surface needed by the unfinished IPC half: `enqueue_pending`, `pending_for_target`, `pending_targets`, `pending_count_total`, `pending_count_for_target`, `pending_count_breakdown`, `delete_rename_op`, `delete_pending_for_target`, `list_recent_rename_ops`, plus `RewriteKind` (`WikiLink` / `Tag` / `BlockRef`, schema repr lowercase-snake), `PendingRewriteRow`, `NewPendingRewrite`, `RenameOpRow`. New `IndexError::UnknownEnum { table, column, value }` surfaces malformed `rewrite_kind` strings without panic. +12 tests in `pending::tests` + 2 in `runner` / `migrations`.
...
`materialize_on_read(idx, path, on_disk) -> Result<String, IndexError>` pulls rows for `path`; empty rows returns input. +35 tests in `vault::pending::tests` covering each kind, composed sequences, multi-byte source, and `materialize_on_read` round-trip.

*Materialize-on-read invariant wired into every effective-content reader.* `read_file_text`, `get_embed`, and `get_unlinked_mentions` all run the on-disk read through `materialize_on_read` before returning (the editor view + embed bodies + unlinked-mention scan reflect post-rename world pre-flush). `get_canonical_ast` derives from `read_file_text`, inherits materialization. The bulk vault scan's pass 1 reads each markdown file once, materializes once, hands the materialized source to all four extractors (refresh_frontmatter + extract_links_from_source + refresh_tags + refresh_blocks). The watcher's `Modified` branch keeps the existing raw read for `content_hash` (its purpose is detecting on-disk change) and adds a second raw-bytes-then-materialize read for the extractors. Pass 2's link resolution is unchanged. `refresh_X` signatures changed from `(vault, abs: &Path, rel)` to `(vault, rel, source: &str)` so the caller does the read+materialize once; `extract_links_off_executor` renamed to `extract_links_from_source`. +7 new tests across the modified handlers + 1 watcher invariant test + 1 scan test.

*`link_mention` flushes before splicing.* When `pending_count_for_target(target=source_file) > 0`, calls a new `crate::commands::rename::flush_target_for_link_mention` helper (apply_pending + atomic_write + delete_pending_for_target + best-effort `files.content_hash` bump) BEFORE re-reading disk and computing the splice. Closes the "splice into materialized but write non-materialized" trap called out in `document-model.md` §5.7. +2 tests in `rename::tests`.

**What did NOT land (chain 4 scope; remaining J.1 work).**

- `mint_rename_op_id` helper (`config['pending_rewrites.next_rename_op_id']` monotonic counter).
- Rename IPC handlers: `rename_file`, `rename_tag`, `rename_block_id`. Including the **explicit rekey** approach for `rename_file` — none of the existing FKs have `ON UPDATE CASCADE` (SQLite default is `NO ACTION`), so the handler must `UPDATE links / tags / blocks / block_refs / frontmatter SET <path-col> = ?to WHERE <path-col> = ?from` BEFORE the `UPDATE files`. This was verified in code during the chain-4 drive; the design spec already calls it out.
- Flush IPCs: `flush_pending_rewrites`, `flush_pending_rewrites_for_target` (the chain-3 stub `flush_target_for_link_mention` is the per-target executor; rename it and re-shim as the IPC). Plus the per-vault `flush_in_progress` mutex.
- Read-only IPCs: `get_pending_rewrites_count`, `get_pending_rewrites_breakdown`, `list_recent_rename_ops`, `undo_rename`.
- Flush triggers: 5-min periodic timer (per-vault tokio task using `tokio_util::sync::CancellationToken` for cancel), mandatory close_vault flush, >50-per-file fuse in each rename handler, manual via IPC.
- Backend own-write hash gate: per-vault `Arc<Mutex<HashSet<(PathBuf, ContentHash)>>>` populated by flush, drained by the watcher dispatcher's `Modified` branch before emitting `vault:file-changed`. Three new fields on `OpenVault` (`flush_own_writes`, `flush_in_progress`, `flush_timer_cancel`); the partial chain-4 attempt added these but didn't migrate the 9 existing `OpenVault { … }` struct-literal call sites, so the additions were reverted to keep the tree green.
- `lib.rs` `generate_handler!` registration of the seven new IPCs (rename × 3, flush × 2, count, breakdown, list-ops, undo — nine total).
- `ui/src/api/ipc.ts` typed binding stubs + the two new listeners (`onVaultPendingRewritesChanged`, `onVaultFlushComplete`) + the `pending_rewrites.flush_interval_secs` entry in the `Setting` union.
- Events: `vault:pending-rewrites-changed { vault_id, count }` + `vault:flush-complete { vault_id, files_rewritten, refs_updated }` emitters.

**Tests.** 326 baseline + 55 new (= 381 Rust). vitest unchanged (329).

**Verification status.** `cargo test --workspace` green at 381. `cargo clippy --workspace --all-targets -- -D warnings` clean. `cargo fmt --all --check` clean. `npx tsc --noEmit` + `npm run build` + `vitest` not re-run this session (no UI changes landed); the chain-3 commits don't touch UI.

**Next.** A fresh window driving [`docs/superpowers/prompts/l3-session-j1-pending-rewrites-backend.md`](../superpowers/prompts/l3-session-j1-pending-rewrites-backend.md) STEP 2 items 5–11 closes J.1. Then J.2 (frontend), then K (L3 closeout).
