...
*Rename IPCs — `cubical-app::commands::rename`.* New module + a transactional `mint_rename_op_id` helper using `config['pending_rewrites.next_rename_op_id']` (first call → 1).
