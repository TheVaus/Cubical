...
**Detail:** `scripts/check.sh` is the list; `.github/workflows/ci.yml` runs it. Unit tests live in `cubical-core`, `cubical-ast` and `cubical-index`; the app crate has integration tests against a temp vault; `ui/` and `design-system/` each run their own vitest.
