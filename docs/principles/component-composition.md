...
**Gate:** `scripts/gates/composition.py` — a per-file ratchet plus a shell rule, both configured in `scripts/component-budgets.json`, which owns the current figures. `App.tsx` is being ratcheted down from the 3274 lines that prompted the rule, with its `./api/` imports waived under issue #86 until vault boot, the watcher listeners and the block-ref calls move out. The document session moved to `core/documentSession.ts` and the file explorer to `explorer/`; the waiver names what is left.
